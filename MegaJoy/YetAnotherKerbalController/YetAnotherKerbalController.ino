...
byte lastToggleValue[8];
unsigned long lastFlipTime[8];
...
megaJoyControllerData_t getControllerData(void){
  ...
  for (int i = 2; i < 10; i++){
    ...
    if (pinVal != lastToggleValue[i-2]) {
      lastToggleValue[i-2] = pinVal;
      lastFlipTime[i-2] = millis() + 100;
    }

    if (lastFlipTime[i-2] > millis()) {
      controllerData.buttonArray[(i/2) / 8] |= (HIGH) << ((i - 2) % 8);
    } else {
